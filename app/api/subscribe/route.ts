...
export async function POST(req: NextRequest) {
  let body: { email?: unknown; frequency?: unknown; category_slugs?: unknown }
  try {
    body = await req.json()
  } catch {
    ...
    await resend?.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:24px">
          ...
          <p>You signed up for <strong>${escapeHtml(frequency)}</strong> Austin events updates for: <strong>${escapeHtml(categoryLabel)}</strong>.</p>
